...
- **The data-source abstraction** — `FruitDataSource` is a protocol with two implementations, `BLEFruitDataSource` for the real rig and `MockFruitDataSource` for an in-process generator. Views never touch CoreBluetooth, so the whole app runs in the Simulator with no hardware attached. This mattered because the rig is a single physical object only one person can hold at a time.
- **The sync pipeline** — `SyncEngine` is the sole consumer of that protocol: it grades incoming events, validates them, persists `FruitRecord` / `RejectedEvent` through SwiftData, and writes the graded result back over BLE.
- **The iPad interface** — login, live grading, dashboard with Swift Charts, batch history, and CSV/PDF export via the native share sheet.
- **The SwiftData schema** and the configurable ID/timestamp sourcing described in `Data_Architecture.md`.
- **The architecture documentation** — the 199-line [`app/biki/README.md`](app/biki/README.md).

**Built by my teammate** — the initial project scaffold and the first pass of the ESP32 firmware.
